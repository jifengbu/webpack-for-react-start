...
export default class TextFormItem extends React.Component {
    ...
    render () {
        const { form, label, value, help, placeholder, editing, readOnly, rows, layout, rules, buttons = [], style, className, required = true, hasFeedback = true, formGroup, ...otherProps } = this.props;
        const key = _.keys(value)[0];
        const option = {};
        if (rows) {
                ...
                <div className={buttons.length ? styles.iconButtonContainer : ''}>
                    ...
                    }
                    {
                        editing && form.getFieldDecorator(key, {
                            ...
                            ||
                            <Input {...otherProps} readOnly={readOnly} placeholder={placeholder || `请输入${label}`} onChange={::this.handleChange} />
                        ) || (
