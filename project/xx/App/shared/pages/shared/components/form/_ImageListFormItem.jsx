...
function getCheckValidator (label, count, noFull) {
...
export default class ImageListFormItem extends React.Component {
    ...
    render () {
        const { form, label, layout, editing, count, value, required = true, classNames = [], noFull = false } = this.props;
        ...
        return (
            <FormItem
                ...
                >
                {editing && form.getFieldDecorator(key, { initialValue: value[key], rules: [ { required, message: `请选择${label}` }, { validator: getCheckValidator(label, count, noFull) } ] })(<span style={{ marginTop: 10 }} />)}
