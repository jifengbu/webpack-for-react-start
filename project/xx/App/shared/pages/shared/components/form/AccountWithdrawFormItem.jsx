...
export default class AccountWithdrawFormItem extends React.Component {
    render () {
        const { account, layout, formGroup, withoutDetail } = this.props;
        const { amount, usableAmount, frozeAmount, bondAmount, lockAmount, strictAmount } = account;
        let children = [];
        let help = [];
        if (!withoutDetail) {
            if (strictAmount > 0) {
                children.push(`定向资金：${_N(strictAmount)} 元`);
        ...
        return (
            <FormItem
                ...
                label='可提现金额'
                >
                { `${_N(Math.max(0, usableAmount - strictAmount))} 元` }
                {
                    !withoutDetail &&
