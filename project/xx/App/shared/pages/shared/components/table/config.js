...
export function fixColumns (columns) {
    ...
    return columns.map((o) => {
        ...
        if (o.buttons) {
            o.key = o.key || 'x';
            o.className = '__filter_click';
            o.render = (data, record, index) => (
                <div className='__filter_click_container __filter_click'>
                    { o.buttons.map((item, key) => (typeof item.visible === 'function' ? item.visible(data, record, index) : item.visible !== false) && <a className='__filter_click_button __filter_click' key={key} onClick={() => item.onClick(data, record, index)} >{item.text}</a>) }
                </div>
            );
        }
        return o;
